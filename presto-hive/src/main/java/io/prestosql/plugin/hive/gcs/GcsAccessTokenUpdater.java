...
import io.prestosql.plugin.hive.HdfsDynamicConfigurationUpdater;
...
public class GcsAccessTokenUpdater
        implements HdfsDynamicConfigurationUpdater
{
    @Override
    public Configuration updateConfiguration(Configuration configuration, HdfsEnvironment.HdfsContext context, URI uri)
    {
        ...
        context.getIdentity().getConnectorTokens().ifPresent(x -> {
            String token = x.get(GCS_ACCESS_KEY_CONF);
            if (token != null) {
                configuration.set(GCS_ACCESS_KEY_CONF, token);
            }
        });
        return configuration;
    }
